...
import { Droplets, Wind, Gauge, AlertTriangle } from "lucide-react"
...
export function WeatherOverview({ defaultLocation = "Cairo", className }: WeatherOverviewProps) {
  ...
  const fetchWeather = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || t("weatherMarket.error"))
      }
  ...
  const summaryText = useMemo(() => {
    ...
    return template
      ...
      .replace("{rain}", today.chanceOfRain != null ? `${today.chanceOfRain}` : "0")
